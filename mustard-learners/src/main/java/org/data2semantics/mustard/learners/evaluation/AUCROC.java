 ...
 *
 */
public class AUCROC implements EvaluationFunction {
	...
	public double computeScore(double[] target, Prediction[] prediction) {
		...
		for (int i = 0; i < target.length; i++) {
			...
			int fold = 1;
			if (prediction[i].getFold() > 0 && !prediction[i].isProbabilities()) {
		...
		for (int fold : foldSizes.keySet()) {
			...
			if (prediction[0].isPairWise()) {
				...
				for (Pair<Double,Double> p : classifiers.keySet()) {
					aucTemp += AUCUtils.computeRocAuc(classifiers.get(p));
				}
				...
				for (Double d : classifiers.keySet()) {
					aucTemp += AUCUtils.computeRocAuc(classifiers.get(d));
				}
