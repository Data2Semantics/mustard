 ...
 *
 */
public class DTGraphWLSubTreeOneGraphKernel implements GraphKernel<SingleDTGraph>, FeatureVectorKernel<SingleDTGraph>, ComputationTimeTracker, FeatureInspector {
	...
	public DTGraphWLSubTreeOneGraphKernel(int iterations, int depth, boolean reverse, boolean iterationWeighting, boolean trackPrevNBH, boolean normalize) {
	...
	public DTGraphWLSubTreeOneGraphKernel(int iterations, int depth, boolean reverse, boolean iterationWeighting, boolean normalize) {
		this(iterations, depth, reverse, iterationWeighting, false, normalize);
	}


	public DTGraphWLSubTreeOneGraphKernel(int iterations, int depth, boolean normalize) {
	...
	public SparseVector[] computeFeatureVectors(SingleDTGraph data) {		
		this.instanceVertices = new ArrayList<DTNode<StringLabel,StringLabel>>();
		this.instanceVertexIndexMap = new HashMap<DTNode<StringLabel,StringLabel>, Map<DTNode<StringLabel,StringLabel>, Integer>>();
		this.instanceEdgeIndexMap = new HashMap<DTNode<StringLabel,StringLabel>, Map<DTLink<StringLabel,StringLabel>, Integer>>();
	...
	private void computeFVs(DTGraph<StringLabel,StringLabel> graph, List<DTNode<StringLabel,StringLabel>> instances, double weight, SparseVector[] featureVectors, int lastIndex, int currentIt) {
		...
		for (int i = 0; i < instances.size(); i++) {
			...
			for (DTNode<StringLabel,StringLabel> vertex : vertexIndexMap.keySet()) {
				...
				if (!trackPrevNBH || !vertex.label().isSameAsPrev()) { 
			...
			for (DTLink<StringLabel,StringLabel> edge : edgeIndexMap.keySet()) {
				...
				if (!trackPrevNBH || !edge.tag().isSameAsPrev()) { //edge are actually at d*2 - 1 
