...
import org.data2semantics.mustard.kernels.graphkernels.singledtgraph.DTGraphWLSubTreeOneGraphKernel;
...
public class RDFWLSubTreeOneGraphKernel implements GraphKernel<RDFData>, FeatureVectorKernel<RDFData>, ComputationTimeTracker, FeatureInspector {
	private int depth;
	private boolean inference;
	private DTGraphWLSubTreeOneGraphKernel kernel;
	private SingleDTGraph graph;

	public RDFWLSubTreeOneGraphKernel(int iterations, int depth, boolean inference, boolean normalize) {
		this(iterations, depth, inference, false, false, normalize);
	}
	
	public RDFWLSubTreeOneGraphKernel(int iterations, int depth, boolean inference, boolean reverse, boolean iterationWeighting, boolean normalize) {
		this(iterations, depth, inference, reverse, iterationWeighting, false, normalize);
	}

	public RDFWLSubTreeOneGraphKernel(int iterations, int depth, boolean inference, boolean reverse, boolean iterationWeighting, boolean trackPrevNBH, boolean normalize) {
		super();
		this.depth = depth;
		this.inference = inference;

		kernel = new DTGraphWLSubTreeOneGraphKernel(iterations, depth, reverse, iterationWeighting, trackPrevNBH, normalize);
	}
