...
import org.data2semantics.mustard.kernels.graphkernels.singledtgraph.DTGraphTreeWLSubTreeOneGraphKernel;
...
public class RDFTreeWLSubTreeOneGraphKernel implements GraphKernel<RDFData>, FeatureVectorKernel<RDFData>, ComputationTimeTracker {
	private int depth;
	private boolean inference;
	private DTGraphTreeWLSubTreeOneGraphKernel kernel;
	private SingleDTGraph graph;

	public RDFTreeWLSubTreeOneGraphKernel(int iterations, int depth, boolean inference, boolean normalize) {
		this(iterations, depth, inference, false, false, false, normalize);
	}

	public RDFTreeWLSubTreeOneGraphKernel(int iterations, int depth, boolean inference, boolean reverse, boolean iterationWeighting, boolean normalize) {
		this(iterations, depth, inference, reverse, iterationWeighting, false, normalize);
	}

	public RDFTreeWLSubTreeOneGraphKernel(int iterations, int depth, boolean inference, boolean reverse, boolean iterationWeighting, boolean trackPrevNBH, boolean normalize) {
		super();
		this.depth = depth;
		this.inference = inference;

		kernel = new DTGraphTreeWLSubTreeOneGraphKernel(iterations, depth, reverse, iterationWeighting, trackPrevNBH, normalize);
	}
