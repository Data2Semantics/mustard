 ...
 (
  ...
  for ((j=1; j<=4; j++)) ; do
     ...
     eval "java -cp mustard-experiments-0.0.1-SNAPSHOT.jar org.data2semantics.mustard.experiments.cluster.ClusterExperiment -leaveRootLabel true -dataset AM $STOPOS_VALUE" 
     stopos remove -p am_approx_pool
	 cp *.result $HOME/exp/am2/results/approx/
  done
) &
done
wait
