 ...
 (
  ...
  for ((j=1; j<=4; j++)) ; do
     ...
     eval "java -cp mustard-experiments-0.0.1-SNAPSHOT.jar org.data2semantics.mustard.experiments.cluster.ClusterExperiment -leaveRootLabel true -dataset BGS $STOPOS_VALUE" 
     stopos remove -p bgs_approx_pool
	 cp *.result $HOME/exp/bgs2/results/approx/
  done
) &
done
wait
