 ...
 (
  ...
  for ((j=1; j<=4; j++)) ; do
     ...
     eval "java -cp mustard-experiments-0.0.1-SNAPSHOT.jar org.data2semantics.mustard.experiments.cluster.ClusterExperiment -dataset BGS -minHubs [40,80,160,320,640,1000000000] -optHubs true $STOPOS_VALUE" 
     stopos remove -p bgs_hub_pool
     cp *.result $HOME/exp/bgs2/results/hub/
  done
) &
done
wait
