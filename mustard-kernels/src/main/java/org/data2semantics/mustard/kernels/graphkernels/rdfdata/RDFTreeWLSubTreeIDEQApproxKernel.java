...
/**
 * Wrapper kernel for {@link org.data2semantics.mustard.kernels.graphkernels.singledtgraph.DTGraphTreeWLSubTreeIDEQApproxKernel}. 
 * 
 * @author Gerben
 */
public class RDFTreeWLSubTreeIDEQApproxKernel implements GraphKernel<RDFData>, FeatureVectorKernel<RDFData>, ComputationTimeTracker, FeatureInspector {
	private int depth;
	private boolean inference;
	private DTGraphTreeWLSubTreeIDEQApproxKernel kernel;
	private SingleDTGraph graph;
	
	public RDFTreeWLSubTreeIDEQApproxKernel(int iterations, int depth, boolean inference, boolean reverse, boolean noDuplicateSubtrees, int[] maxPrevNBHs, int[] maxLabelCards, int[] minFreqs, boolean normalize) {
		super();
		this.depth = depth;
		this.inference = inference;

		kernel = new DTGraphTreeWLSubTreeIDEQApproxKernel(iterations, depth, reverse, noDuplicateSubtrees, maxPrevNBHs, maxLabelCards, minFreqs, normalize);
	}
